...
<p>
  When enabled, AWS Valkyrie attempts to determine the IP address of the client connected through the front-facing proxy, or series of proxies. The `req.ips` property, then contains an array of IP addresses the client is connected through. To enable it, use the values described in the <a href="#trust.proxy.options.table">trust proxy options table</a>.
</p><p>
...
<b>NOTE</b>: Sub-apps <i>will</i> inherit the value of this setting, even though it has a default value.
  ...
  `x-powered-by`
      ...
      <td>Boolean</td>
      <td>Enables the "X-Powered-By: AWS Valkyrie" HTTP header.</td>
  <td markdown="1">
  `true`
  </td>
    </tr>
  ...
  <p markdown="1">
  Read [AWS Valkyrie behind proxies](/guide/behind-proxies.html) for more
  information.
  </p>
...
The [aws-valkyrie.static](#aws-valkyrie.static) middleware ignores these settings.
</p>
